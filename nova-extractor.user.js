...
(function nova() {
    ...
    */
    ...
    function isnovaExtractorHostPage(url) {
    ...
    /*
    	==========================================
    	 MAIN program - determine which scenario to run
    	==========================================
    */

    if (isNovaPublicationPage(url)) {
        novaPublicationPage();
    } else if (isnovaExtractorHostPage(url)) {
...
/*
	==========================================
	 Functions needed on the pages that are mined for data
	==========================================
*/      
		...
		var remove_elements = [
			...
			'#hioa-toolbar',
			'.clearfloat',
			'h4 + .publication_right_content_line',
			'.publication_right_content_line + h4'

			];

		// remove parents of these elements
		var remove_parents = [
			'#download_for_free',
			'.publication_right_content_line > h4'
			];
		...
		for (var i = 0; i < remove_elements.length; i++) {
			var current_element = document.querySelector(remove_elements[i]);
			while(current_element !== null) {
				current_element.parentNode.removeChild(current_element);
				current_element = document.querySelector(remove_elements[i]);
			}
		}

		for (var j = 0; j < remove_parents.length; j++) {
			var current_element = document.querySelector(remove_parents[j]);
			while(current_element !== null) {
				current_element.parentNode.parentNode.removeChild(current_element.parentNode);
				current_element = document.querySelector(remove_parents[j]);
			}
		}
	...
	function getPublisherItem () {
		...
		var pubTypeSegment = rp_ems[1].textContent.trim();
		if(pubTypeSegment) {
    ...
    function getYear() {
	    ...
	    function hasUtgivelsesAarMeta() {
	        var prcl_ems = document.querySelectorAll('.publication_right_content_line em');
	        if (prcl_ems.length == 0) {
	            return false;
	        }
	        for (var i = 0; i < prcl_ems.length; i++) {
	            var current_em = prcl_ems[i];
	            console.log(current_em.textContent.trim());
	            if (current_em.textContent.trim() == "Utgivelsesår:" && current_em.nextElementSibling.textContent.trim().match(/^\d\d\d\d$/)) {
	                return true;
	            }
	        }
	        return false;
	    ...
	    function getUtgivelsesAarMeta() {
	        ...
	        for (var i = 0; i < prcl_ems.length; i++) {
	            var current_em = prcl_ems[i];
	            if (current_em.textContent.trim() == "Utgivelsesår:" && current_em.nextElementSibling.textContent.trim().match(/^\d\d\d\d$/)) {
	                return current_em.nextElementSibling.textContent.trim();
	            }
	        }
	        return false;
	    }

	    function hasYearInPublisherItem(str) {
	    	if(str.match(/\d+\/\d\d+/)) {
				return true;
			} else {
				return false;
	    ...
	    function getYearInPublisherItem(str) {
	    	var yearInPubType = "";
	    	if(str.match(/\d+\/\d\d+/)) {
				yearInPubType = str.replace(/^([^/]+)\/(\d+)$/g,'$2');
			} else {
				return false;
			}
			if(yearInPubType.length == 2) {
				yearInPubType = "20" + yearInPubType;
			}
			return yearInPubType;
	    }

    if (hasYearSegmentInUrl(url)) {
        return getYearFromNovaUrl(url);
    } else if (hasUtgivelsesAarMeta()) {
        return getUtgivelsesAarMeta();
    } else if (hasYearInPublisherItem(getPublisherItem())) {
        return getYearInPublisherItem(getPublisherItem());
    }
}
...
*/
    ...
    function novaExtractorHostPage() {
        console.log('novaExtractorHostPage');
...
*/
        ...
        notater = notater.sort();

        var temahefter = ["http://www.hioa.no/Om-HiOA/Senter-for-velferds-og-arbeidslivsforskning/NOVA/Publikasjonar/Temahefte/Veiledning-og-veiledningsmodeller-i-barnevernets-foerstelinjetjeneste",
            ...
            "http://www.hioa.no/Om-HiOA/Senter-for-velferds-og-arbeidslivsforskning/NOVA/Publikasjonar/Temahefte/Ungdomskulturer-og-narkotikabruk",
        ...
        temahefter = temahefter.sort();

        var skriftserie = ["http://www.hioa.no/Om-HiOA/Senter-for-velferds-og-arbeidslivsforskning/NOVA/Publikasjonar/NOVAs-skriftserie/2005/Two-technical-choices-with-critical-implications",
            ...
            "http://www.hioa.no/Om-HiOA/Senter-for-velferds-og-arbeidslivsforskning/NOVA/Publikasjonar/NOVAs-skriftserie/2000/Commitment-to-welfare-a-question-of-trust",
        ...
        var rapporter = [
            ...
            "http://www.hioa.no/Om-HiOA/Senter-for-velferds-og-arbeidslivsforskning/NOVA/Publikasjonar/Rapporter/2015/Ungdata.-Nasjonale-resultater-2014",
...
/*
	==========================================
	 Functions needed on the host page
	==========================================
*/
        ...
        var lokale_serier = {
            notater: genLocalLinks(notater),
            rapporter: genLocalLinks(rapporter),
            skriftserie: genLocalLinks(skriftserie),
